...
async def start_user_bot(bot_id: int, code: str, user_bot_token: str) -> bool:
    ...
    try:
        ...
        # Простой wrapper для запуска кода пользователя
        wrapper = '''#!/usr/bin/env python3
"""Wrapper для запуска кода пользователя"""
import os
import sys
import signal
import traceback
...
try:
    ...
    exec(code_obj, {"__name__": "__main__", "__file__": os.path.abspath("user_bot.py")})
    log("Код выполнен")
except SystemExit as e:
    log(f"Бот завершился с кодом: {e.code}")
    sys.exit(e.code if e.code else 0)
except ImportError as e:
    log(f"❌ ОШИБКА ИМПОРТА: {e}")
    log("Установлю библиотеку и попробую снова...")
    try:
        # Пробуем установить по имени модуля
        module_name = str(e).split("'")[1] if "'" in str(e) else str(e).split()[-1]
        log(f"Устанавливаю: {module_name}")
        subprocess.check_call([sys.executable, "-m", "pip", "install", module_name])
        log("Установлено! Перезапустите бота.")
    except Exception as install_err:
        log(f"Не удалось установить: {install_err}")
    sys.exit(1)
